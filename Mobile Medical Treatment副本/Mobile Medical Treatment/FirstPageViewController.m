...
- (void)viewDidLoad {
    ...
    self.title = @"移动医疗";
    UIBarButtonItem *rightButton = [[UIBarButtonItem alloc]initWithImage:[FirstPageViewController scale:[UIImage imageNamed:@"message_white_icon"] toSize:CGSizeMake(25, 25)] style:UIBarButtonItemStylePlain target:self action:@selector(changeToMessageTable:)];
...
//    _tableView.delegate = self;
    ...
    type2.name = @"医生咨询";
    type2.detail = @"咨询指定医生";
    type2.image = [UIImage imageNamed:@"yishengzixun"];
    
    FunctionType *type3 = [[FunctionType alloc]init];
    type3.name = @"门诊缴费";
    type3.detail = @"缴费方便快捷";
    type3.image = [UIImage imageNamed:@"mode2"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.row) {
        ...
        default:
            [(AppDelegate *)[UIApplication sharedApplication].delegate fillViews:3];
            break;
    }
}
